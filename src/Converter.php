...
class Converter extends Runner
{
	...
	{
		if (!app()->isAlias($adapter)) {
			throw new AdapterException("Adapter {$adapter}  is not Supported.");
		}

		$handle = app($adapter);

		$this->filesystem = new Filesystem($handle->pick());

		$this->driver = $handle->getDriver();
